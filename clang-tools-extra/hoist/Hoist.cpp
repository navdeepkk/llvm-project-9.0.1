...
public:
  ...
  virtual void run(const MatchFinder::MatchResult &Result) {
    ...
    if (const LabelStmt *ls =
      ...
      locls = locls.substr(locls.find(':') + 1, locls.find(':'));
      locls = locls.substr(0, locls.find(':'));
      Rewrite.ReplaceText(ls->getBeginLoc(),
                          lsname + "( struct s_" + lsname + "* __s )");
    }
  }
    ...
    if (const LabelStmt *lp =
      ...
      if (const RecordDecl *rd =
        ...
        if (locrd.find("invalid") == std::string::npos &&
          ...
          // sm->getDecomposedSpellingLoc(loc);
...
public:
  MyASTConsumer(Rewriter &R)
      : labelBuilder(R), labelRelBuilder(R), labelRenamer(R), labelHoist(R),
        labelRemover(R), recordMatcher(R), structNotRemove(),
        functionDumper(R) {
    ...
    LabelRenameFinder.addMatcher(
        labelStmt(isExpansionInMainFile()).bind("stmt"), &labelRenamer);
    // first finding nested structures only parent should be replaced.
    StructNotRemoveFinder.addMatcher(
    ...
    // blank string.
    StructRemoveFinder.addMatcher(
        recordDecl(hasAncestor(labelStmt().bind("parent")),
                   isExpansionInMainFile())
            .bind("child"),
        &recordMatcher);
    StructRemoveFinder2.addMatcher(
    ...
    LabelHoistFinder.addMatcher(
        labelStmt(hasAncestor(functionDecl().bind("parent")),
                  isExpansionInMainFile())
            .bind("child"),
        &labelHoist);
    LabelHoistFinder.addMatcher(
        labelStmt(hasAncestor(labelStmt().bind("parent")),
                  isExpansionInMainFile())
            .bind("child"),
        &labelHoist);
    // remove label stmts from the code.
    LabelRemoveFinder.addMatcher(
        labelStmt(isExpansionInMainFile()).bind("stmt"), &labelRemover);
